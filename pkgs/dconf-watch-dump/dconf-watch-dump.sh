...
dconf dump / > "$before"
log "Snapshot taken. Make your changes now."
log "Press ENTER when you're done..."
read -r
dconf dump / > "$after"

log "==== DCONF diff ===="
log "(check if something was removed here but isn't in the DUMP at the end)"
log "(if so, manually remove them from the config file)"
